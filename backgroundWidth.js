...
function createImageEffect(setting, defaultValue, minValue, maxValue, increment, humanReadable) {
	...
	$( '#' + setting + '_effect' ).before('<span>'+humanReadable+':</span>').after('<input id="'+setting+'_slider" type="range" min="'+minValue+'" max="'+maxValue+'" step="'+increment+'" /><br>');
	...
	$('#'+setting+'_slider').mouseup(function() {
		console.log(currentlyEditingbackground);
